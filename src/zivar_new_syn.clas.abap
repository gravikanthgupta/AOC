CLASS zivar_new_syn DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zivar_new_syn IMPLEMENTATION.
  ...
  METHOD if_oo_adt_classrun~main.
    ...
    zivar_new_syn=>s1_using_key_expression(   ).

    zivar_new_syn=>s1_constructor_expression(  ).


    zivar_new_syn=>s1_table_expression(
      IMPORTING
    ...
    zivar_new_syn=>s1_inline_declaration(  ).

    zivar_new_syn=>s1_value_expression(  ).
    out->write( im_game ).

    zivar_new_syn=>s1_corresponding_data(  ).
    out->write( im_game2 ).

    zivar_new_syn=>s1_cond_conv_exp(  ).
    out->write( im_res ).

    zivar_new_syn=>s1_loop_with_grouping(  ).
    out->write( im_grp_book ).

    zivar_new_syn=>s1_loop_with_single_line(  ).
    out->write( im_final_booking ).

    zivar_new_syn=>s1_loop_reduce_statement(  ).
    out->write( im_total ).
