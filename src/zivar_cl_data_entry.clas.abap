CLASS zivar_cl_data_entry DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
*        output =
  ...
  METHOD fill_master_data.
    data : lt_bp type table of zivar_r_bpa,
           lt_prod type table of zivar_r_product.
   ...
   append value #(
    ...
    append value #(
     ...
     insert zivar_r_bpa from table @lt_bp.
  ...
  METHOD fill_transaction_data.
    ...
    select * from zivar_r_bpa into table @data(lt_bpa).
    select * from zivar_r_product into table @data(lt_prod).
    ...
    insert zivar_r_so_hdr from table @lt_so.
    insert zivar_r_so_item from table @lt_so_i.
